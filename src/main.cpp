...
void Main()
{
	...
	while (System::Update())
	{
		...
		if (type == 1)	boldFont(U"EASY").draw(100, Vec2{ 50, 60 }, ColorF{ 0.85,0.6,0.73 });
		else if (type == 2)	boldFont(U"NORMAL").draw(100, Vec2{ 50, 60 }, ColorF{ 0.3,0.56,0.23 });
		else if (type == 3)	boldFont(U"HARD").draw(100, Vec2{ 50, 60 }, ColorF{ 0.68,0.26,0.15 });
		else if (type == 4)	boldFont(U"INSANE").draw(100, Vec2{ 50, 60 }, ColorF{ 0.36,0.06,0.45 });
		...
		if (start) {
			...
			if (SeventInterval <= SaccumulatedTime) break;
		}
	}
